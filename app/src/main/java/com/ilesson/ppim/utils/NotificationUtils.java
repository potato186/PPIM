...
/**
 * Created by potato on 2019/8/15.
 */
...
public class NotificationUtils {
    ...
    private static NotificationManager getManager() {
        if (manager == null) {
            manager = (NotificationManager) IlessonApp.getInstance().getSystemService(NOTIFICATION_SERVICE);
        }
        return manager;
    }
    ...
    private static Notification.Builder getNotificationBuilder(String title, String content, String channelId) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            //id随便指定
            NotificationChannel channel = new NotificationChannel(channelId, IlessonApp.getInstance().getPackageName(), NotificationManager.IMPORTANCE_DEFAULT);
        ...
        return new Notification.Builder(IlessonApp.getInstance()).setAutoCancel(true).setChannelId(channelId)
                .setContentTitle(title)
                .setContentText(content).setSmallIcon(R.mipmap.icon);
    }
