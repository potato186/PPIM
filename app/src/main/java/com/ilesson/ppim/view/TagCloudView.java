...
//    private int mRightImageResId;
    ...
    private static final int DEFAULT_TAG_RESID = R.layout.tagcloud_item;
...
//        mTagBorderHor = PPScreenUtils.dip2px(context,10);
...
//            imageView.setImageResource(mRightImageResId);
    ...
    private void setTagsState(int index){
    ...
    private void setTagsWidth(){
        final int sw = PPScreenUtils.getScreenWidth(getContext());
        for (int i = 0; i < textViews.size(); i++) {
            final TextView textView = textViews.get(i);
            textView.post(new Runnable() {
                @Override
                public void run() {
                    if(textView.getWidth()>0.92*sw){
                        LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
                        layoutParams.width= (int) (0.92*sw);
