...
@MessageTag(value = "custom:order", flag = MessageTag.ISCOUNTED | MessageTag.ISPERSISTED)
public class NewOrderMessage extends MessageContent {
    ...
    * */
    public NewOrderMessage(byte[] data) {
        String jsonStr = null;
    ...
    //给消息赋值。
    public NewOrderMessage(Parcel in) {
     ...
     */
    public static final Creator<NewOrderMessage> CREATOR = new Creator<NewOrderMessage>() {

        @Override
        public NewOrderMessage createFromParcel(Parcel source) {
            return new NewOrderMessage(source);
        }

        @Override
        public NewOrderMessage[] newArray(int size) {
            return new NewOrderMessage[size];
        }
    };
