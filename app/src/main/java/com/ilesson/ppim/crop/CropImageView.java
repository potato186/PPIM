...
import com.ilesson.ppim.crop.animation.SimpleValueAnimator;
import com.ilesson.ppim.crop.animation.SimpleValueAnimatorListener;
import com.ilesson.ppim.crop.animation.ValueAnimatorV14;
import com.ilesson.ppim.crop.animation.ValueAnimatorV8;
import com.ilesson.ppim.crop.callback.Callback;
...
import com.ilesson.ppim.crop.util.Logger;
import com.ilesson.ppim.crop.util.Utils;
...
public class CropImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    private void setupLayout(int viewW, int viewH) {
        if (viewW == 0 || viewH == 0||mIsCropping) return;
    ...
    private Bitmap decodeRegion() {
        ...
        try {
            ...
            int minLen = Math.min(originalImageHeight, originalImageWidth); // 原图的最小边长
    ...
    private void saveToFile(Bitmap bitmap, final String path) {
        OutputStream outputStream = null;
//        String path = MyFileUtils.getRealImagePathFromURI(getContext(),uri);
...
//                    .openOutputStream(uri);
            ...
            Logger.e("An error occurred while saving the image: " + path, e);
        ...
        mHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mSaveCallback != null) mSaveCallback.onSuccess(path);
            }
        });
    }
    ...
    public Bitmap getCircularBitmap(Bitmap square) {
        ...
        float scale = halfHeight/halfWidth;
//        if(halfWidth>1000){
//            halfWidth=1000;
//            halfHeight = (int) (halfWidth*scale);
//        }
     ...
     */
    public void startCrop(String saveUri, CropCallback cropCallback, SaveCallback saveCallback) {
        mSaveUri = saveUri;
